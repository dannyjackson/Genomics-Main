...
This script sorts, RGADDs and dupmarks BAM outputs from either BWA or Dragmap alignment pipelines.
...
printf "\n\n\n\n"
date
echo "Current script: A0.2_bam_alignment_sorting.sh"
...
if [ -f "${OUTDIR}/datafiles/sortedbamfiles/${IND}/temp" ];
        then
            echo "temp directory for ${IND} sortedbamfile already exists, moving on!"
        else
        mkdir -p "${OUTDIR}/datafiles/sortedbamfiles/${IND}/temp"
fi
