...
usage() {
    ...
    echo ""
    echo "This script phases a VCF file with SHAPEIT5 using a VCF and mask generated through the SNPable pipeline."
...
while read -r SCAFFOLD; do
    ...
    # Run shapeit phasing
    ${PROGDIR}/shapeit_bins/phase_common_static --input "$VCF_IN" --region 1 --map info/chr1.gmap.gz --output "$VCF_OUT" --thread $THREADS
