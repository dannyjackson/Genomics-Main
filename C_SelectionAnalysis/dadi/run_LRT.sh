...
python3 scripts/Genomics-Main/C_SelectionAnalysis/dadi/dadi_run_LRT.py -j ${JOB_NAME} -s ${SFS_PATH} -o ${OUTDIR} -b ${BOOT_DIR}  -i "${NESTED_INDICES}" -n ${NULL_MODEL} -t ${TEST_MODEL} --null_popt "${NULL_POPT}" --test_popt "${TEST_POPT}"
