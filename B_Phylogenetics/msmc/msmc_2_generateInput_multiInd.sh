...
for s in `cat SCAFFOLDS.txt`
        ...
        for ind in $IND
                do INDMASK=`ls ${OUTDIR}/mask/ind/ind_mask.${ind}.${SCAFFOLD}.bed.gz`
                echo "--mask=$INDMASK " >> ${OUTDIR}/mask/ind/${POP}.mask_file.$SCAFFOLD
                INDVCF=`ls ${OUTDIR}/vcf2/${ind}.${SCAFFOLD}.phased.vcf.gz`
                echo $INDVCF >> ${OUTDIR}/vcf2/${POP}.vcf_file.${SCAFFOLD}
        done
...
### Generate MSMC input files:
        if [ $METHOD == samtools ]
                ...
                msmc-tools/generate_multihetsep.py `cat ${OUTDIR}/mask/ind/${POP}.mask_file.${SCAFFOLD}` --mask=$MASK_GENOME `cat ${OUTDIR}/vcf2/${POP}.vcf_file.${SCAFFOLD}` > ${MSMC_INPUT} # without repeat mask
                ...
                msmc-tools/generate_multihetsep.py --mask=$MASK_GENOME `cat ${OUTDIR}/vcf/${POP}.vcf_file.${SCAFFOLD}` > $MSMC_INPUT # without repeat mask
                ...
                msmc-tools/generate_multihetsep.py --mask=$MASK_INDIV --mask=$MASK_GENOME $VCF > $MSMC_INPUT # with new repeat mask
        fi
