#!/bin/bash

# all parameters come from the msmc_param control file
# make edits there before using this script!

# Check for at least one argument (parameter file path)
if [ $# -lt 1 ]; then
    echo "Usage: $0 -p <path_to_parameter_file>"
    echo "This script generates input files for MSMC."
    echo "Required Argument:"
    echo "  -p   Path to msmc parameter file"
    echo "  -i   Individual or Population Name"
    exit 1
fi

# Parse command-line arguments
while getopts ":p:i:" opt; do
    case "${opt}" in
        p) PARAMS=${OPTARG} ;;
        i) POP_OR_IND=${OPTARG} ;;
        *) echo "Invalid option: -${OPTARG}"
           exit 1 
           ;;
    esac
done

# Ensure parameter file is provided and exists
if [ -z "${PARAMS}" ]; then
    echo "Error: No parameter file provided." >&2
    exit 1
elif [ ! -f "${PARAMS}" ]; then
    echo "Error: Parameter file '${PARAMS}' not found." >&2
    exit 1
fi

source "${PARAMS}"


if [ -d "${OUTDIR}/datafiles/bootstraps/${POP_OR_IND}" ]; then
    echo "${POP_OR_IND} bootstrap input directory found"
else
    mkdir -p ${OUTDIR}/datafiles/bootstraps/${POP_OR_IND}/log_and_loop
fi

#input for the bootstrapping
BS_INPUT=`for s in $(cat ${OUTDIR}/referencelists/SCAFFOLDS.txt); do find ${OUTDIR}/datafiles/msmc/input/ -maxdepth 1 -name "msmc_input.*${POP_OR_IND}.${s}*.txt"; done`

echo "Bootstrap_Input: ${BS_INPUT}"

#output from the bootstrapping 
BS_OUTPUT=${OUTDIR}/datafiles/bootstraps/${POP_OR_IND}/${POP_OR_IND}.bootstrap
echo $BS_OUTPUT

echo "generating bootstraps for ${IND}"
${PROGDIR}/msmc-tools/multihetsep_bootstrap.py --out_dir_prefix $BS_OUTPUT --files $BS_INPUT

ls -d *${OUTDIR}/datafiles/bootstraps/${POP_OR_IND}/${POP_OR_IND}.bootstrap_* > ${OUTDIR}/datafiles/bootstraps/boots_file_lsts/${POP_OR_IND}.bs_list.txt

echo "Finished generating bootstraps for ${POP_OR_IND}"

# Run MSMC on all Bootstrapped datasets for your given pop/individual by iteratively sending batch jobs with msmc_4_bootstraps.sh
echo "Submitting slurm array to run MSMC on newly generated Bootstraps for ${POP_OR_IND}"

sbatch --account=mcnew \
	--job-name=bootsrap_msmc_array.${POP_OR_IND} \
    --partition=windfall \
	--output=bootstrap_msmc_${POP_OR_IND}.out \
	--nodes=1 \
	--ntasks=${THREADS} \
	--time=40:00:00 \
    --array=1-20 \

    BOOT="$( sed "${SLURM_ARRAY_TASK_ID}q;d" ${OUTDIR}/datafiles/bootstraps/boots_file_lsts/${POP_OR_IND}.bs_list.txt )"
    ${SCRIPTDIR}/Genomics-Main/B_Phylogenetics/msmc/msmc_4_run_bootstraps.sh -p ${PARAMS} -b ${BOOT} -i ${POP_OR_IND}