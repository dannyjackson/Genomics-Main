...
echo 'Removing prior output files'
if [ -f "OUTPUT_$FILE" ]
then
rm OUTPUT_$FILE
fi
...
echo 'Copying  map and ped to data.map & data.ped'
...
#echo "Submitting GONEparallel via slurm"
#echo "Running GONEparallel via slurm" >> timefile

#sbatch --account=mcnew \
#	--job-name=gone_parallel \
#    --partition=standard \
#	--output=gone_parallel.out \
#	--nodes=1 \
#	--ntasks=1 \
#	--time=01:00:00 \
#	./PROGRAMMES/GONEparallel.sh -hc $hc outfileLD $REPS $FILE

###################################################################

./PROGRAMMES/GONEparallel_original.sh -hc $hc outfileLD $REPS
